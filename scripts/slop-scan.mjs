#!/usr/bin/env node
/**
 * slop-scan.mjs — deterministic metrics for the slop-check skill.
 *
...
const fileData = new Map(); // rel -> data
const tellExamples = [];
const dupIndex = new Map(); // hash -> Set<rel>
const dupWindowsByFile = new Map(); // rel -> [{hash, startLine}]
...
for (const f of files) {
  ...
  const codeLinesNorm = []; // [normalizedText, lineNo]
...
// Duplication totals
let dupLines = 0, totalCodeLines = 0, dupFilePairs = new Set();
for (const [rel, data] of fileData) {
