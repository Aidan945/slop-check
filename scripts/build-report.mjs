#!/usr/bin/env node
/**
 * build-report.mjs — inject a report payload into the HTML template.
 *
...
need(Number.isFinite(payload.slopPercent) && payload.slopPercent >= 0 && payload.slopPercent <= 100, 'slopPercent must be a number 0-100');
...
let injected = null;
try { injected = JSON.parse(landed[1]); } catch { /* handled below */ }
