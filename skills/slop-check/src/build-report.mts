...
const dataPath: string | undefined = flag('data');
const outPath: string = resolve(flag('out', 'slop-report.html')!);
const templatePath: string = resolve(flag('template', join(here, '..', 'assets', 'report-template.html'))!);
...
let payload: Payload;
try {
  payload = JSON.parse(readFileSync(resolve(dataPath), 'utf8')) as Payload;
} catch (e) {
  console.error(`build-report: payload is not valid JSON: ${(e as Error).message}`);
  process.exit(1);
}

// Validate just enough to keep a broken report from reaching the user.
const problems: string[] = [];
const need = (cond: unknown, msg: string): void => { if (!cond) problems.push(msg); };
...
const weightSum: number = (payload.categories ?? []).reduce((s, c) => s + (c.weight ?? 0), 0);
...
for (const f of payload.findings ?? []) {
  ...
  need(['minor', 'moderate', 'major'].includes(f.severity as string), `finding at ${f.file} has invalid severity "${f.severity}"`);
}
...
const template: string = readFileSync(templatePath, 'utf8');
...
const out: string = template.replace(
  ANCHOR,
  '$1/*SLOP_DATA*/' + JSON.stringify(payload, null, 2) + '/*END_SLOP_DATA*/',
);
...
let injected: Payload | null = null;
try { injected = JSON.parse(landed![1]) as Payload; } catch { /* handled below */ }
